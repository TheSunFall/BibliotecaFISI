...
public class VerificadorInicioSesion {
    public Usuario Verificador(String correo, char[] password) {
        ...
        try (InputStream input = getClass().getResourceAsStream("/usuarios.txt");
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (datos.length >= 6) {
                    try {
                        if (datos[3].equals(correo) && datos[1].equals(new String(password))) {
                            return new Usuario(
                                datos[2], // nombre
                                datos[3], // correo
            ...
            e.printStackTrace(); // Agregar impresión de stack trace para depuración
        }
